...
const createWeb3State = ({ web3, provider, isLoading, contractNFT, contractMarketplace, contractAuction }) => {
  return {
    web3,
    provider,
    isLoading,
  ...
  React.useEffect(() => {
    const loadProvider = async () => {
      ...
      if (provider) {
        ...
        const {contractNFT, contractMarketplace, contractAuction} = await loadContract(web3);
          setWeb3Api(
          createWeb3State({
            web3,
            provider,
